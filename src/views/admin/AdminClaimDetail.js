...
const AdminClaimDetail = () => {
    ...
    useEffect(() => {
        ...
        setAdminNote((currentValue) => (currentValue ? currentValue : request.adminNote));
    }, [request?.adminNote]);
    ...
    const handleApprove = () => {
        Alert.alert(
            ...
            [
                ...
                { text: isAffiliationHelp ? 'Traiter' : 'Accepter', onPress: runPrimaryAction },
            ],
        );
    };
    ...
    const handleRefuse = () => {
        Alert.alert(
            ...
            [
                { text: 'Annuler', style: 'cancel' },
                { text: 'Refuser', style: 'destructive', onPress: runSecondaryAction },
            ],
        );
    };
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={[ApplicationStyle.card, Spaces.padding[20], Spaces.marginBottom[16]]}>
                    ...
                    <View style={[Alignments.row, Alignments.alignCenter]}>
                        <ProfileAvatar size={60} imageUrl={requester?.avatar} />
                    ...
                    <View style={[ApplicationStyle.card, Spaces.padding[20], Spaces.marginBottom[16]]}>
                        ...
                        <View style={[Spaces.gap[10]]}>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Type: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>{request.__typeLabel}</Text>
                            </Text>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Nom recherché: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>{request.clubName || '-'}</Text>
                            </Text>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Source: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>{request.source || '-'}</Text>
                            </Text>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Recherche initiale: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>
                                    {request?.searchContext?.currentQuery || '-'}
                                </Text>
                            </Text>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Date: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>{requestDate}</Text>
                            </Text>
                            <Text style={[Fonts.p2, Fonts.neutral200]}>
                                Commentaire: <Text style={[Fonts.p2Bold, Fonts.neutral00]}>{request.comment || '-'}</Text>
                            </Text>
                        </View>
                    </View>
                ) : (
