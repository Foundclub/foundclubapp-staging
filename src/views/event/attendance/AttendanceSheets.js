...
/**
 * 2F — CORRIGER UN POINTAGE.
 *
 * 🗑️ Le geste destructeur (« Dépointer ») est en `error300`, jamais en bouton
 * plein : un pointage se defait, mais ca ne doit pas etre le geste le plus
 * facile de la feuille.
 *
 * 🧨 « Annuler la note du staff » RENVOIE LE RETARD COURANT. `patchLate` exige
 * `lateMinutes` : envoyer 0 pour effacer une note effacerait aussi le retard.
 * @param {object} props - Les proprietes de la feuille.
 * @param {boolean} props.isVisible - La feuille est-elle ouverte.
 * @param {any} props.item - La ligne visee.
 * @param {() => void} props.onChangeTime - Ouvrir 2E en correction.
 * @param {() => void} props.onClearNote - Effacer la note du staff.
 * @param {() => void} props.onClose - Fermer.
 * @param {() => void} props.onUnmark - Depointer.
 * @param {(key: string, fallback: string) => string} props.t - Le traducteur.
 * @returns {import('react').ReactElement} - La feuille.
 */
export function AttendanceCorrectSheet({
  isVisible,
  item,
  onChangeTime,
  onClearNote,
  ...
  const { Colors, Fonts } = useTheme();
  const aUneNote = Boolean(item?.attendance?.note);

  const actions = [
    {
      color: Colors.neutral00,
      key: 'time',
      label: t('eventDetails.attendanceCall.correct.changeTime', "Changer l'heure d'arrivée"),
      onPress: onChangeTime,
    },
    ...(aUneNote ? [{
      color: Colors.neutral00,
      key: 'note',
      label: t('eventDetails.attendanceCall.correct.clearNote', 'Annuler la note du staff'),
      onPress: onClearNote,
    }] : []),
    {
      color: Colors.error300,
      key: 'unmark',
      label: t('eventDetails.attendanceCall.correct.unmark', 'Dépointer — remettre en attente'),
      onPress: onUnmark,
    },
    {
      color: Colors.neutral300,
      key: 'close',
      label: t('eventDetails.attendanceCall.correct.close', 'Fermer'),
      onPress: onClose,
    },
  ];
      ...
      <View style={styles.body}>
        {actions.map((action) => (
          <TouchableOpacity
            ...
            style={styles.action}
          >
            <Text style={[Fonts.p2Bold, { color: action.color }]}>{action.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </BottomModal>
